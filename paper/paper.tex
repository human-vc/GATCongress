...
The SRI's temporal pattern corroborates the era decomposition in Section~8: the transitions with the highest SRI (111th--112th, 110th--111th) coincide with the era when bridge legislator elimination was most intense, while the low post-2013 SRI values reflect a network that has already settled into a stable disconnected configuration with little structural change between sessions.
...
This does not diminish BLI as a descriptive measure. The spectral analysis, null models, and structural recovery findings are unaffected. What the centrism control clarifies is mechanism: moderates are electorally vulnerable because they are moderate, and their departure damages connectivity because moderates are the members whose voting patterns span partisan clusters. The structural consequence is real; the pathway is ideological.
...
The House co-voting network collapsed from a Fiedler peak of 0.805 (107th Congress) to 0.032 (118th), with a previously undocumented spike to 0.737 in the 111th immediately before the Tea Party wave shattered it. The recovery ratio declined from 1.93 (Contract with America) to 0.01 (Tea Party): the system lost its capacity to absorb structural shocks. Roughly 35 bridge legislators account for most of the structural difference between the connected 103rd and the disconnected 118th, but their electoral vulnerability is driven by ideological centrism rather than network position per se.
